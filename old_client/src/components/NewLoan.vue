      ...
      <v-layout row wrap>
        <v-flex xs6 offset-xs3>
          <v-toolbar flat dense class="blue" dark>
            <v-toolbar-title white>Insert a new loan</v-toolbar-title>
          </v-toolbar>
  
          <v-flex xs12 sm6 md12 style="padding-left: 50px; padding-right: 50px;">
            <form name="loan-add-form" autocomplete="off">
                ...
                <v-layout wrap align-center>
                    <v-flex xs6 sm3 md5>
                        <!-- <v-text-field label="From"  v-if="lenterIsUser" placeholder="User e-mail or username" required :rules="[required]" type="text" v-model="Loan.UserLenterId"></v-text-field> -->
              ...
              <!-- <v-select v-if="selectFrom=='Me'" :items="Books" label="Book" v-model="bookNamesSelect"></v-select> -->
            ...
            <br>
            <v-btn color="primary" v-if="!lenterIsUser && lenterIsUser != null" dark @click="dialogBook = true">Edit book </v-btn> 
            <v-btn dark class="cyan" @click="addLoan">
              Submit
            </v-btn>
          </v-flex>
        </v-flex>
      </v-layout>
      <!-- <v-dialog v-model="dialogLenter" persistent max-width="290">
        <v-card>
          <v-card-title class="headline">Book lent</v-card-title>
          <v-card-text>The user that is lenting you the book is already a member of myLibrary application?</v-card-text>          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat @click="lenterIsUserMethod">Yes</v-btn>
            <v-btn color="green darken-1" flat @click="lenterIsNotUserMethod">No</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-dialog v-model="dialogBorrower" persistent max-width="290">
        <v-card>
          <v-card-title class="headline">Book lent</v-card-title>
          <v-card-text>The user that you are lenting the book to is already a member of myLibrary application?</v-card-text>          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat @click="borrowerIsUserMethod">Yes</v-btn>
            <v-btn color="green darken-1" flat @click="borrowerIsNotUserMethod">No</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog> -->
...
<script>
  import NewBookManuallyPanel from './NewBookManuallyPanel'
